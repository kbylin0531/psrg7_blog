...
/**
 * 标签model
 * @method TagModel getInstance() static
 * @method  array|string getFieldByTid(int $tid, string $field)
 */
Class TagModel extends BaseModel
{
    ...
    public function getAllData()
    {
        $data = $this->select();
        foreach ($data as $k => $v) {
            $data[$k]['count'] = ArticleModel::getInstance()->where(array('tid' => $v['tid']))->count();
        }
        return $data;
    }
