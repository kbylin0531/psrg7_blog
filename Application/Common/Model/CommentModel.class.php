...
/**
 * 评论model
 * @method CommentModel getInstance() static
 * @method array|string|int getFieldByCmtid($id,$field)
 */
class CommentModel extends BaseModel
{
    ...
    public function addData($type)
    {
        ...
        $is_admin = OauthUserModel::getInstance()->getFieldById($ouid, 'is_admin');
        ...
        if (C('COMMENT_SEND_EMAIL') && $is_admin == 0) {
            ...
            if (!empty($address)) {
                $title = ArticleModel::getInstance()->getFieldByAid($data['aid'], 'title');
...
html;
        ...
        if (C('COMMENT_SEND_EMAIL') && $data['pid'] != 0) {
            ...
            $parent_data = OauthUserModel::getInstance()->field('nickname,email')->find($parent_uid);
        ...
        foreach ($data as $k => $v) {
            ...
            if (!empty($child)) {
                ...
                foreach ($child as $m => $n) {
                    ...
                    // 获取被评论人昵称
                    $child[$m]['reply_name'] = OauthUserModel::getInstance()->getFieldById($reply_user_id, 'nickname');
                }
            }
            $data[$k]['child'] = $child;
        }
